//
//  OwnerDetailsViewModelTests.swift
//  GitHubReposTests
//
...
class OwnerDetailsViewModelTests: XCTestCase {

    var sut: OwnerDetailsViewModel!

    override func setUp() {
        super.setUp()
        let coordinator = AppCoordinator(navigationController: UINavigationController())
        let repo = Repo()
        sut = Assembler.sharedAssembler.resolver.resolve(OwnerDetailsViewModel.self, arguments: coordinator, RepositoryType.mock, repo)!
    }
